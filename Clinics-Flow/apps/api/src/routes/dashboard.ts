...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    const whatsappSent = whatsappStats.find((s: any) => s.status === 'sent')?._count.status || 0;
    const whatsappDelivered = whatsappStats.find((s: any) => s.status === 'delivered')?._count.status || 0;
    const whatsappFailed = whatsappStats.find((s: any) => s.status === 'failed')?._count.status || 0;
...
router.get('/whatsapp/stats', async (req, res, next) => {
  try {
    ...
    const totals = messageStats.reduce(
      (acc: { sent: number; delivered: number; failed: number }, curr: any) => {
